...
# Reuse PEM implementation (registered in mmcv's registry)
# No longer reused from pem_v7, inlined into this file for easier debugging
...
# PEM-related custom modules
@ATTENTION.register_module(name='SPOT_CA')
class SPOT_CA(nn.Module):
    """
    ...
    def forward(self,
        ...
        if cross_attn_mask is not None:
            ...
            # Fill with a very small value where the mask is True, 
...
@TRANSFORMER_LAYER.register_module(name='SparseProtoTransformerDecoderLayer')
class SparseProtoTransformerDecoderLayer(nn.Module):
    """
    ...
    def __init__(self,
        ...
        self.spot_cross_attn = SPOT_CA(**spot_ca_cfg)
        ...
        # Three LayerNorms: after cross_attn, self_attn, and ffn
        self.norms = ModuleList()
        for _ in range(3):
            self.norms.append(LayerNorm(spot_ca_cfg['embed_dims']))

        self.embed_dims = spot_ca_cfg['embed_dims']
...
class SparseProtoMask2FormerOpenOccHead(MaskFormerHead):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 dn_cfg=None,
                 dn_loss_layers=1, # Number of layers to compute DN loss
                 **kwargs):
        ...
        # Compatible with PEM decoder layer config, 
        # automatically parse num_heads from spot_ca_cfg/self_attn_cfg
        first_layer_cfg = transformer_decoder['transformerlayers']
        if 'spot_ca_cfg' in first_layer_cfg and 'num_heads' in first_layer_cfg['spot_ca_cfg']:
            self.num_heads = first_layer_cfg['spot_ca_cfg']['num_heads']
        ...
        # create class_weights for nuScenes
        ...
        loss_cls.class_weight = self.class_weight
         ...
         # Add DN (Denoising) config and components 
        if dn_cfg is None:
            dn_cfg = {
                'scalar': 1,  # number of repetitions for each GT mask
                'noise_scale': 0.2,  # intensity of feature noise
                'dn_label_noise_ratio': 0.2  # ratio of noise to add to GT labels
            }
        self.dn_cfg = dn_cfg
        self.dn_loss_layers = dn_loss_layers
        
        # Add a class embedding layer to convert GT class labels into feature vectors
        ...
        # for sparse segmentation (keep interface for config compatibility)
    ...
    def _prepare_for_dn(self, gt_labels_list, gt_masks_list):
        ...
        # If there are no GTs, return None to skip DN
        ...
        for batch_idx, (gt_labels, gt_masks) in enumerate(zip(gt_labels_list, gt_masks_list)):
            ...
            if num_gts == 0:
                # If no GT, create a zero-filled mask for padding.
                # Spatial dimensions need to be obtained from other batches or a fixed size.
                # Default dimensions are used here and will be interpolated later based on actual feature map size
                w, h, d = 64, 64, 5  # default spatial dimensions, will be interpolated later based on actual feature map size
            ...
            # Add point noise to masks
            # Ensure mask is boolean type
            masks_bool = repeated_masks.bool()
            # Get spatial dimensions of the mask: W, H, D
            _, W, H, D = gt_masks.shape
            
            # Calculate the volume of each mask (number of True voxels)
            areas = masks_bool.sum(dim=(-1, -2, -3))
            
            # Calculate noise ratio based on volume and noise scale factor
            ...
            # GT=1, delta=1 -> 0 (create holes in the object)
            # GT=0, delta=1 -> 1 (create noise in the background)
            noised_masks = torch.logical_xor(masks_bool, delta_mask)
            
            # Create a padding tensor to ensure a consistent number of masks per batch
            ...
            # Add label noise: randomly replace a portion of GT labels with other classes
            ...
            # Fill DN features into corresponding positions
        ...
        # Permute to (num_dn_queries, batch_size, feat_channels) to match Transformer input format
        dn_queries = dn_queries.permute(1, 0, 2)
        
        # Construct self-attention mask
        # total queries = DN queries + original learnable queries
        total_queries = pad_size + self.num_queries
        tgt_mask = torch.ones(total_queries, total_queries).cuda() < 0  # Initialize as False (visible)
    ...
    def loss(self, all_cls_scores, all_mask_preds, gt_labels_list,
        ...
        Args:
            all_cls_scores (Tensor):
                Classification scores for all decoder
                layers with shape (num_decoder_layers, batch_size, total_queries,
        ...
        # Calculate loss for each decoder layer independently
        num_dec_layers = len(all_cls_scores)
        # Create same gt_labels_list and gt_masks_list for each decoder layer
        all_gt_labels_list = [gt_labels_list for _ in range(num_dec_layers)]
        all_gt_masks_list = [gt_masks_list for _ in range(num_dec_layers)]
        img_metas_list = [img_metas for _ in range(num_dec_layers)]
        
        # Calculate losses for matching part (using existing Hungarian matcher logic)
        ...
        # Calculate losses for DN part (if DN queries exist)
        ...
        # DN loss for the last decoder layer (if exists and is supervised)
        ...
        for i in range(num_dec_layers - 1):  # All layers except the last one
            ...
            # DN loss (if exists)
            if pad_size > 0 and i < self.dn_loss_layers:
    ...
    def loss_single(self, cls_scores, mask_preds, gt_labels_list,
        ...
        Returns:
            tuple[Tensor]: Loss components for outputs from a single \
                decoder layer.
        """
        num_imgs = cls_scores.size(0)  # Split by batch size dimension
        cls_scores_list = [cls_scores[i] for i in range(num_imgs)]
        mask_preds_list = [mask_preds[i] for i in range(num_imgs)]

        # Assign targets for each prediction, includes labels, label_weights, 
        ...
        # Stack gt labels and mask weights for each query
        # shape (batch_size, num_queries)
        labels = torch.stack(labels_list, dim=0)
        # shape (batch_size, num_queries)
        label_weights = torch.stack(label_weights_list, dim=0)
        # Special handling for mask_targets: concat positive masks of all images
        ...
        # Handle no positive samples
        if mask_targets.shape[0] == 0:
            # Zero match: loss is 0 when no positive samples
            # Set dummy values for Dice loss and Mask loss (using sum of predictions)
            # Return losses early to avoid calculation errors
        ...
        '''
        # Randomly sample K points for supervision, which can improve efficiency.
        
        with torch.no_grad():
            # Sample points based on prediction uncertainty and class frequency
            ...
            # Get gt labels on sampled points
            # use gather to extract labels for sampled points from mask_targets
            mask_point_targets = torch.gather(mask_targets.view(mask_targets.shape[0], -1), 
                                        dim=1, index=point_indices)
        
        # Get predictions on sampled points
    ...
    def loss_dn_single(self, cls_scores, mask_preds, gt_labels_list,
        ...
        """
        Calculate loss for Denoising queries.
        DN queries do not require Hungarian matching, loss is calculated directly based on GT.
        
        Args:
            cls_scores (Tensor): Classification scores for DN part, shape (batch_size, pad_size, num_classes+1)
            mask_preds (Tensor): Mask predictions for DN part, shape (batch_size, pad_size, w, h, d)
        ...
        Returns:
            tuple[Tensor]: DN loss (loss_cls_dn, loss_mask_dn, loss_dice_dn)
        """
        ...
        for batch_idx, (gt_labels, gt_masks) in enumerate(zip(gt_labels_list, gt_masks_list)):
            ...
            # Construct DN labels for current batch (repeated scalar times)
            batch_dn_labels = torch.full((pad_size,), self.num_classes, dtype=torch.long, device=device)  # Initialize to background class
            batch_dn_label_weights = torch.zeros((pad_size,), dtype=torch.float, device=device)
            
            # Construct DN mask targets and weights for current batch
            batch_dn_masks = []
            batch_dn_mask_weights = torch.zeros((pad_size,), dtype=torch.float, device=device)
            
            if num_gts > 0:
                # Fill in real DN labels and masks
                for i in range(scalar):
                    start_idx = i * max_num_gts
                    end_idx = start_idx + num_gts
                    ...
                    # Add GT masks to batch mask list
                    for gt_mask in gt_masks:
                        batch_dn_masks.append(gt_mask)
                        
                # Fill remaining positions with zero masks
                ...
                # No GT case, fill with zero masks
        ...
        if mask_preds_valid.shape[0] == 0:
            # No valid masks, return zero loss
            ...
            with torch.no_grad():
                # --- New code: Prepare aligned GT for sampling ---
                # dn_masks_valid shape is (num_valid, w, h, d)
                # Sampling function needs a list of tensors, each tensor representing GT masks for a sample
                # So we split dn_masks_valid into a list, each element shape is (1, w, h, d)
                ...
                point_indices, point_coords = get_uncertain_point_coords_3d_with_frequency(
                    ...
                    None, 
                    dn_labels_list_for_sampling,  # <--- Use new GT labels list
                    dn_masks_list_for_sampling,   # <--- Use new GT masks list
                    self.sample_weights, 
                    self.num_points, 
            ...
            # Get predicted values for sampled points
            mask_point_preds = point_sample_3d(
    ...
    def forward_head(self, 
        ...
        # PEM version no longer constructs/uses cross-attention attn_mask
    ...
    def forward_train(self,
        ...
        # forward - 将 gt_labels 和 gt_masks 传递给 forward 函数以支持 DN
    ...
    def forward(self, 
        ...
        # Remove coarse branch, directly use highest resolution features

        # Use all voxel features of the highest resolution, no need for sampling
        ...
        dn_meta = {}     # DN metadata, empty dictionary during inference
        ...
        if self.training and gt_labels_list is not None and gt_masks_list is not None:
            ...
            if dn_result is not None:
                ...
                # query_embed also needs to be concatenated, DN part uses DN queries as positional embeddings
        ...
        cls_pred_list = []
        mask_pred_list = []
        # In training, all decoder layers (including the initial one) need supervision;
        # in inference, only keep the final layer to avoid redundant large volume mask predictions.
        if self.training:
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            # Generate cross-attention mask for current decoder layer
            cross_attn_mask = None
            if self.training and dn_result is not None and dn_gt_masks is not None and i < self.dn_loss_layers:
                # Get spatial dimensions of current feature map
                ...
                # Get spatial dimensions of original GT masks
                # dn_gt_masks: (batch_size, pad_size, w, h, D)
                gt_w, gt_h, gt_d = dn_gt_masks.shape[-3:]
                
                # Calculate kernel size and stride
                # PyTorch's Conv3D/MaxPool3D dimension order is (d, h, w)
                # But (w, h, d) is also acceptable, as long as kernel_size and stride correspond
                ...
                # Since max_pool results are 0 or 1, using < 0.5 is robust
                
                # Flatten and reverse logic: True means masked (not attended), False means can be attended
                dn_mask_flat = (downsampled_dn_masks < 0.5).flatten(2)  # (batch_size, pad_size, X*Y*Z)
                
                # Create a mask for matching queries (all False, meaning can attend to all positions)
                ...
                # Concatenate to get the full cross-attention mask
            ...
            # In training: output layer by layer; in inference: output only once at the last layer
        ...
        Returns:
            ...
            - mask_pred_list (list[Tensor]): Mask logits for each \
                decoder layer. Each with shape (batch_size, num_queries, \
                 X, Y, Z).
            - dn_meta (dict): DN metadata dictionary, containing pad_size etc., empty during inference
        '''
    ...
    # Coarse voxel loss branch removed